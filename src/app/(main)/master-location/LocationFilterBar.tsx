...
function FilterInner({ warehouses, role }: { warehouses: any[], role: string }) {
  ...
  // State lokal: Mengambil nilai default dari URL jika ada
  const [searchTerm, setSearchTerm] = useState(searchParams.get('q') || '')
  const [selectedWh, setSelectedWh] = useState(searchParams.get('wh') || '')

  // Fungsi saat tombol Apply diklik
  const handleApply = () => {
    ...
    // Memicu refresh data server dengan URL parameter baru
  ...
  const handleReset = () => {
    setSearchTerm('')
    setSelectedWh('')
    router.replace(pathname) // Bersihkan semua parameter URL
  }
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-3 w-full bg-slate-50 p-4 rounded-xl border border-slate-200 mb-6 sm:items-end">
      ...
      {/* 1. Input Search Nama Rak */}
      <div className="flex-1 w-full">
        <label className="text-xs font-semibold text-slate-500 mb-1.5 block">Cari Nama / Barcode Rak</label>
        <div className="relative">
          ...
          <input
            ...
            onKeyDown={(e) => e.key === 'Enter' && handleApply()} // Bisa tekan enter untuk apply
            ...
            placeholder="Ketik lalu klik Apply..."
          />
        </div>
      </div>

      {/* 2. Select Warehouse (Hanya muncul untuk Superadmin) */}
      {role === 'superadmin' && (
...
export default function LocationFilterBar({ warehouses, role }: { warehouses: any[], role: string }) {
  return (
